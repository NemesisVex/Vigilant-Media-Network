...
<h2 style="padding:20px 0px 10px 0px; border-bottom:1px dashed #ccc; margin-bottom:10px;">File Downloads</h2>
<?php if ($downloads): ?>
<table class="cart_table" cellpadding="0" cellspacing="0" border="0">
	...
	foreach($downloads as $download): ?>
		...
		<tr class="cart_item">
			<td>
				<?php $d = format_date($download->ordered_on); 
			...
			<td><a href="<?php echo site_url('secure/download/' . $download->file_order_token . '/'); ?>"><?php echo (!empty($download->file_label)) ? $download->file_label : $download->file_name; ?></a> (<?php echo ceil((filesize($download->file_path . '/' . $download->file_name)/1024)/1024); ?>MB)</td>
		</tr>
...
<?php else: ?>
	You have no downloads in your account history.
<?php endif; ?>
