...
<article>
	<header>
		<div class="span-14 last source-label">
			<h3 class="source-title"><a href="/index.php/sakufu/entry/{$rsEntry->entry_id}/">{$rsEntry->entry_title}</a></h3>
		</div>

		<div class="span-4">
			<p>
			<time datetime="{$rsEntry->entry_created_on|date_format:"%Y-%m-%dT%H:%M:%S-06:00"}" pubdate="pubdate">
{$rsEntry->entry_created_on|date_format:"%b %d, %Y %H:%M"}
			</time>
			</p>

			<p>
				<em>By {$rsEntry->author_name}</em><br>
{if $rsEntry->category_id}Filed: <a href="/index.php/sakufu/category/{$rsEntry->category_id}/">{$rsEntry->category_label}</a><br>{/if}
			</p>
...
{/if}
			<strong>Share:</strong>
			<a href="http://www.facebook.com/sharer.php?u={'http://www.gregbueno.com/index.php/sakufu/entry/'|cat:$rsEntry->entry_id|cat:'/'|escape:'url'}" title="[Share on Facebook]"><img src="{$config.to_vigilante}/images/icons/facebook.png" alt="[Facebook]" /></a>
			<a href="http://twitter.com/share?url={'http://gregbueno.com/index.php/sakufu/entry/'|cat:$rsEntry->entry_id|cat:'/'|escape:'url'}&amp;text={'「作譜」: '|escape:'url'}{$rsEntry->entry_title|truncate:'75'|escape:'url'}" title="[Share on Twitter]"><img src="{$config.to_vigilante}/images/icons/twitter.png" alt="[Twitter]" /></a>
			<a href="http://del.icio.us/post" onclick="window.open('http://del.icio.us/post?v=4&amp;noui&amp;jump=close&amp;url='+encodeURIComponent(location.href)+'&amp;title='+encodeURIComponent(document.title), 'delicious','toolbar=no,width=700,height=400'); return false;"><img src="{$config.to_vigilante}/images/icons/delicious.png" alt="[Delicious]" /></a>
		</p>
...
<nav class="entry-nav">
{if $rsNext}<a href="/index.php/sakufu/entry/{$rsNext->entry_id}/" title="{$rsNext->entry_excerpt}">NEXT</a>{else}NEXT{/if} &#8226;
{if $rsPrevious}<a href="/index.php/sakufu/entry/{$rsPrevious->entry_id}/" title="{$rsPrevious->entry_excerpt}">PREVIOUS</a>{else}PREVIOUS{/if} &#8226;
	<a href="/index.php/sakufu/entry/random/">RANDOM</a><br>
</nav>
