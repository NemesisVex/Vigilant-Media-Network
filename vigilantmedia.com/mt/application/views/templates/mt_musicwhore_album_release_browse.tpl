...
{if $rsReleases}
<p>Choose a release to edit{if $asin} with Amazon information for <strong>{$album_title}</strong>{/if}:</p>
...
<tr>
	<td valign="top">
	<strong><a href="/index.php/musicwhore/release/{$function}/{$rsRelease->release_id}/{$album_artist_id}/{if $asin}{$asin}/{/if}">{$rsRelease->album_title}</a></strong> {if $rsRelease->release_alt_title}<em>({$rsRelease->release_alt_title})</em>{/if}<br>
	</td>
