...
class Ep4_release_model extends CI_Model
{
	...
	function get_entry_by_track_id($track_id)
	{
		...
		$this->db->where('c.content_track_id', $track_id);
		$this->db->where('cat.category_id', $this->category_wrp_track);
	...
	{
		$this->_build_base_content_query();
		$this->db->where('c.content_entry_id', $entry_id);
	...
	{
		$this->_build_base_content_query();
		$this->db->where('c.content_release_id', $release_id);
		$this->db->where('c.content_track_id', 0);
		$this->db->where('cat.category_id', $this->category_wrp_release);
