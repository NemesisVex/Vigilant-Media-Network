...
{if $rsTrack->track_audio_is_downloadable==true}
								<a href="/index.php/music/save/{$rsTrack->track_id}/" class="log_audio_download" title="Download"><img src="/images/icons/download-music-grey.gif" width="14" height="14" alt="[Download]" border="0"></a>
{else}
{assign var=track_num value=$rsTrack->track_track_num}
{if $rsITunesLinks.$track_num}
								<a href="{$rsITunesLinks.$track_num|escape:'html'}" class="itunes_download" title="Download from iTunes"><img src="/images/icons/download-music-grey.gif" width="14" height="14" alt="[Download from iTunes]" border="0"></a>
{else}
...
{assign var=exm_file value=$config.ep4_cover_root_path|cat:"/_exm_front_342_"|cat:$rsRelease->album_image}
						<img src="/images/_covers/_{if file_exists($wrp_file)}exm{else}color{/if}_front_200_{$rsRelease->album_image}" alt="[{$rsRelease->album_title}]" title="[{$rsRelease->album_title}]" class="album-cover" width="200" height="200"/><br/>
