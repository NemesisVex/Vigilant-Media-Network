...
class Fw extends CI_Controller
{
	...
	function __construct()
	{
		...
		$this->mwlib->mw_config['site_name'] = 'Filmwhore.org';
		
		if (false !== ($rowFilms = $this->Mw_film_model->get_all_films_group_by_initial()))
		{
			$rsFilmNav = $this->vigilantedblib->_db_return_smarty_array($rowFilms);
			$this->mysmarty->assign('rsFilmNav', $rsFilmNav);
		}
	}
	...
	function index()
	{
		...
		$page_config['base_url'] = '/index.php/fw/index/';
		...
		$this->mwlib->_smarty_display_fw_page('fw_root_index.tpl');
	}
	
	function about()
	{
		$this->mwlib->_format_section_head('About');
		$this->mwlib->_smarty_display_fw_page('fw_root_about.tpl');
	}
	
	function contact()
	{
		$this->mwlib->_format_section_head('Contact');
		$this->mwlib->_smarty_display_fw_page('fw_root_contact.tpl');
	}
	
	function contact_sent()
	{
		$this->mwlib->_format_section_head('Contact');
		$this->mwlib->_smarty_display_fw_page('mw_root_contact_sent.tpl');
	}
	
	function terms()
	{
		$this->mwlib->_format_section_head('About', 'Terms and conditions of use');
		$this->mwlib->_smarty_display_fw_page('fw_root_terms.tpl');
	}
	
	function error($code)
	{
		$this->mwlib->_format_section_head('Error', $code, $this->error_codes[$code]);
		$this->mwlib->_smarty_display_fw_page('mw_error_' . $code . '.tpl');
	}
	...
	function category($category_id)
	{
		...
		$page_config['base_url'] = '/index.php/fw/category/' . $category_id . '/';
		
		$this->_display_news_page($rowEntries, $offset, 4, $page_config);
	}
	
	function title($filter = 'a')
	{
		$segment = 4;
		$offset = $this->uri->segment($segment);
		
		$this->mwlib->_format_section_head('Movies', strtoupper($filter));
		
		$rowEntries = $this->Mw_content_model->get_content_by_film_initial($filter, $this->blog_id);
		
		$page_config['base_url'] = '/index.php/fw/title/' . $filter . '/';
		
		$this->_display_news_page($rowEntries, $offset, $segment, $page_config);
	}
	...
	function date($month = '', $year = '')
	{
		...
		$page_config['base_url'] = '/index.php/fw/date/' . $month . '/' . $year . '/';
	...
	function entry($entry_id)
	{
		...
		if (false !== ($rowMaps = $this->Mw_content_model->get_maps_by_entry_id($entry_id)))
		{
			if ($rowMaps->num_rows() == 1)
			{
				...
				if (false !== ($rsFilm = $this->Mw_film_model->get_film_by_id($rsMap->content_film_id)))
				{
					$this->mysmarty->assign('rsFilm', $rsFilm);
					
					if (false !== ($rsLinks = $this->mwlib->_build_buy_links($rsMap->content_film_id, 'us', 'film_id')))
					{
						$this->mysmarty->assign('rsLinks', $rsLinks);
					}
					
					if (!empty($rsFilm->film_asin_num))
					{
						$amazon_url = $this->myamazon->build_amazon_url($rsFilm->film_asin_num, 'us');
						$this->mysmarty->assign('amazon_url', $amazon_url);
						
						$items = $this->_lookup($rsFilm->film_asin_num, 'us', 'Large');
		...
		$this->mwlib->_smarty_display_fw_page('fw_root_entry.tpl');
	}
	...
	function email()
	{
		...
		'subject' => 't',
		'message' => 'b');
		$this->mwlib->email($hidden_fields, $shown_fields, $this->mwlib->mw_config['site_name'], '/index.php/fw/contact/sent/');
	}
	
	// Private methods
	function _display_news_page($rowEntries, $offset, $segment, $page_config, $display_page = 'fw_root_browse.tpl')
	{
		$page_config['total_rows'] = $rowEntries->num_rows();
		$page_config['per_page'] = $this->per_page;
		$page_config['uri_segment'] = $segment;
		...
		$this->mysmarty->assign('page_links', $page_links);
		$this->mysmarty->assign('rsEntries', $rsEntries);
		$this->mwlib->_smarty_display_fw_page($display_page);
	}
