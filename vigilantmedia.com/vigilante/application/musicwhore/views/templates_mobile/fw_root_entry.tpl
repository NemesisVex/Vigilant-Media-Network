...
{/if}
		...
		<p>
			Share:
			<a href="http://www.facebook.com/sharer.php?u={'http://www.filmwhore.org/index.php/fw/entry/'|cat:$entry_id|cat:'/'|escape:'url'}" title="[Share on Facebook]"><img src="{$config.to_vigilante}/images/icons/facebook.png" alt="[Facebook]" /></a>
			<a href="http://twitter.com/share?url={'http://filmwhore.org/index.php/fw/entry/'|cat:$entry_id|cat:'/'|escape:'url'}&amp;text={'Filmwhore.org: '|escape:'url'}{$rsEntry->entry_title|truncate:'75'|escape:'url'}" title="[Share on Twitter]"><img src="{$config.to_vigilante}/images/icons/twitter.png" alt="[Twitter]" /></a>
...
{/if}
	...
	<p>
{if $rsFilm->film_imdb_id}<em><a href="http://us.imdb.com/title/{$rsFilm->film_imdb_id}/" class="normal">{if $rsFilm->film_title_prefix}{$rsFilm->film_title_prefix} {/if}{$rsFilm->film_title}</a></em>{else}<em>{if $rsFilm->film_title_prefix}{$rsFilm->film_title_prefix} {/if}{$rsFilm->film_title}</em>{/if}{if $rsFilm->film_release_date}: {$rsFilm->film_release_date|date_format:"%Y"}{/if}
{if $rsFilm->film_director_name}, dir. {$rsFilm->film_director_name}{/if}.
{if $rsFilm->film_viewed_type} {$rsFilm->film_viewed_type}{if $rsFilm->film_viewed_date} ({$rsFilm->film_viewed_date|date_format:"%b. %d, %Y"}){/if}.{/if}<br>
	</p>
...
<div class="indent">
	...
	<script type="text/javascript" src="{$config.to_vigilante}/includes/jquery.cookie.js"></script>
{literal}
