<?php
class Ddn_mt_model extends CI_Model
{
	...
	function get_entry_by_id($entry_id, $blog_id)
	{
		$this->db->from($this->entry_table);
		$this->db->where('entry_blog_id', $blog_id);
		$this->db->where($this->entry_table_index, $entry_id);
		if (false !== ($rowEntry = $this->db->get()))
		{
	...
	function get_adjacent_entry($date, $blog_id, $order_by = 'asc')
	{
		...
		$this->db->from($this->entry_table);
		$this->db->where('entry_blog_id', $blog_id);
		$this->db->where('entry_created_on ' . $direction . ' ' . $this->db->escape($date));
		$this->db->order_by('entry_created_on', $order_by);
		$this->db->limit(1);
		if (false !== ($rowEntry = $this->db->get()))
		{
			$rs = $this->vigilantedblib->_db_return_rs($rowEntry);
	...
	function get_latest_entries($blog_id, $limit = 5)
	{
		...
		$entryQuery .= 'Order By e.entry_created_on Desc ';
	...
	function get_entries_by_year($y = 2008, $blog_id)
	{
		...
		$entryQuery .= 'Where Year(e.entry_created_on)=' . intval($y) . ' ';
		$entryQuery .= 'And e.entry_status=2 ';
	...
	function get_entries_on_this_day($date, $blog_id)
	{
		...
		$entryQuery .= 'Where (Month(entry_created_on)=' . $month . ' ';
		$entryQuery .= 'And Dayofmonth(entry_created_on)=' . $day . ' ';
		$entryQuery .= 'And Year(entry_created_on)<>' . $year. ') ';
		$entryQuery .= 'And entry_blog_id=' . $blog_id . ' ';
		$entryQuery .= 'And entry_status=2 ';
		$entryQuery .= 'Order By entry_created_on Desc';
	...
	function get_calendar($blog_id, $include_month = true, $limit_year = '')
	{
		...
		$calendarQuery .= 'Order By Year(entry_created_on) Desc, Month(entry_created_on) ';
