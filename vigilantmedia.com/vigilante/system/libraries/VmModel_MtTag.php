...
class VmModel_MtTag extends VmModel {
	...
	public function get_entry_tags($entry_id, $return_results = false) {
		...
		$this->db->join($this->objecttag_table_name, $this->table_name . '.tag_id = ' . $this->objecttag_table_name . '.objecttag_tag_id');
		$this->db->$where_method('objecttag_object_id', $entry_id);
		$this->db->where('objecttag_object_datasource', 'entry');
